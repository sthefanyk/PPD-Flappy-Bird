...
let velocidadeX = 0; // Velocidade de movimento dos canos para a esquerda
...
function obterParametroDaURL(nomeParametro) {
    ...
    return params[nomeParametro];
}

let quantiaAposta = parseFloat(obterParametroDaURL('aposta')) || 0;
...
function iniciarJogo() {
    quantiaAposta = document.getElementById("quantiaAposta").value;
    ...
    window.location.href = `jogo.html?aposta=${quantiaAposta}`;
}
...
function moverPassaro(evento) {
    ...
    if (evento.code == "Space" || evento.code == "ArrowUp" || evento.code == "KeyX") {
        ...
        velocidadeY = -6 - (quantiaAposta / 100) * 0.5; // aumenta 0.2 dependendo a cada 100 reais
    }
}
...
function atualizar() {
    ...
    if (iniciou) {
        ...
        const pontuacaoMonetaria = pontuacao * 0.1;
        contexto.fillText(`R$ ${pontuacaoMonetaria.toFixed(2)}`, 5, 45);
...
function ajustarDificuldade() {
    velocidadeX = -2 - (quantiaAposta / 100) * 0.5;
}
